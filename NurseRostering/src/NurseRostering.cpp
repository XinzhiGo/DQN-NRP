...
const NurseRostering::ShiftID NurseRostering::Scenario::Shift::ID_BEGIN =
    NurseRostering::Scenario::Shift::ID_NONE + 1;

const NurseRostering::SkillID NurseRostering::Scenario::Skill::ID_BEGIN = 0;

NurseRostering::NurseRostering() {
    names.shiftMap[NurseRostering::Scenario::Shift::NAME_ANY] =
        NurseRostering::Scenario::Shift::ID_ANY;
    names.shiftMap[NurseRostering::Scenario::Shift::NAME_NONE] =
        NurseRostering::Scenario::Shift::ID_NONE;
}

void NurseRostering::adjustRangeOfTotalAssignByWorkload() {
    for (NurseID nurse = 0; nurse < scenario.nurseNum; ++nurse) {
        const Scenario::Contract &c(
        ...
        bool ignoreMinShift =
            ((history.currentWeek * c.maxShiftNum) < weekToStartCountMin);
        scenario.nurses[nurse].restMinShiftNum =
            (ignoreMinShift) ? 0
                             : (c.minShiftNum - history.totalAssignNums[nurse]);
#else
        scenario.nurses[nurse].restMinShiftNum =
            c.minShiftNum - history.totalAssignNums[nurse];
#endif
        scenario.nurses[nurse].restMaxShiftNum =
            c.maxShiftNum - history.totalAssignNums[nurse];
        scenario.nurses[nurse].restMaxWorkingWeekendNum =
            c.maxWorkingWeekendNum - history.totalWorkingWeekendNums[nurse];
    }
}
...
void NurseRostering::Penalty::setRepairMode(ObjValue WeightOnUnderStaff,
                                            ObjValue WeightOnSuccesion,
                                            ObjValue softConstraintDecay) {
    modeStack.push_back(pm);

    pm.underStaff = WeightOnUnderStaff;
    pm.succession = WeightOnSuccesion;
    pm.insufficientStaff =
        DefaultPenalty::InsufficientStaff / softConstraintDecay;
    pm.consecutiveShift =
        DefaultPenalty::ConsecutiveShift / softConstraintDecay;
    pm.consecutiveDay = DefaultPenalty::ConsecutiveDay / softConstraintDecay;
    pm.consecutiveDayOff =
        DefaultPenalty::ConsecutiveDayOff / softConstraintDecay;
    ...
    pm.totalWorkingWeekend =
        DefaultPenalty::TotalWorkingWeekend / softConstraintDecay;
}
