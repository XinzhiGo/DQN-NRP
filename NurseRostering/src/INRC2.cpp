...
void help() {
    cout
        << "command line arguments ([XXX] means XXX is optional) :\n"
           "  [id]      - identifier of the run which will be recorded in log "
           "file.\n"
           ...
           "              reaching either timeout or iter will end the "
           "program.\n"
           ...
           "                      'w'(Random Walk), 'i'(Iterative Local "
           "Search),\n"
           "                      'p'(Tabu Search Possibility), 'l'(TS Loop),\n"
           "                      'r'(TS Rand), 's'(Swap Chain) or 'b'(Bias "
           "TS).\n"
           "                  i is a non-negative integer in enum "
           "Solution::ModeSeq.\n"
           "                  next real number is the coefficient of no "
           "improve count.\n"
           "                  following 4 real numbers are coefficients for "
           "TableSize,\n"
           "                  NurseNum, WeekdayNum and ShiftNum used in day "
           "tabu\n"
           "                  tenure setting, non-positive number means there "
           "is \n"
           "                  no relation with that feature. while next 4 "
           "numbers are\n"
           "                  used in shift tabu tenure setting with same "
           "meaning.\n"
...
int run(std::map<std::string, std::string> &argvMap) {
    ...
    // load history last for it will initialize some assist data depending on
    // scenario and weekdata
    if (argvMap.find(ARGV_CUSTOM_INPUT) != argvMap.end()) {
        ...
        spdlog::error("missing obligate argument(history)");
        return -1;
    }

    // check solution file name
    if (argvMap.find(ARGV_SOLUTION) == argvMap.end()) {
        spdlog::error("missing obligate argument(solution file name)");
        return -1;
    }
    ...
    if (argvMap.find(ARGV_TIME) != argvMap.end()) {
        ...
        iss >> timeout;
        input.timeout =
            Timer::Duration(static_cast<int>(
                timeout *
                chrono::duration_cast<Timer::Duration>(chrono::seconds(1))
                    .count())) -
            NurseRostering::Solver::SAVE_SOLUTION_TIME;  // convert second to
                                                         // clock count
    } else {
...
bool readScenario(const std::string &scenarioFileName, NurseRostering &input) {
    ...
    scenario.skillSize =
        scenario.skillTypeNum + NurseRostering::Scenario::Skill::ID_BEGIN;
    input.names.skillNames.resize(scenario.skillSize);
    for (NurseRostering::SkillID i = NurseRostering::Scenario::Skill::ID_BEGIN;
         i < scenario.skillSize; ++i) {
    ...
    scenario.shiftSize =
        scenario.shiftTypeNum + NurseRostering::Scenario::Shift::ID_BEGIN;
    scenario.shifts.resize(scenario.shiftSize);
    input.names.shiftNames.resize(scenario.shiftSize);
    for (NurseRostering::ShiftID i = NurseRostering::Scenario::Shift::ID_BEGIN;
         i < scenario.shiftSize; ++i) {
    ...
    scenario.shifts[NurseRostering::Scenario::Shift::ID_NONE]
        .illegalNextShiftNum = 0;
    for (NurseRostering::ShiftID i = NurseRostering::Scenario::Shift::ID_BEGIN;
         i < scenario.shiftSize; ++i) {
        NurseRostering::Scenario::Shift &shift = scenario.shifts[i];
    ...
    for (NurseRostering::ContractID i = 0; i < contractNum; ++i) {
        ...
        ifs >> input.names.contractNames[i] >> c        // name (
            ...
            >> contract.maxWorkingWeekendNum >> contract.completeWeekend;
...
bool readHistory(const std::string &historyFileName, NurseRostering &input) {
    ...
    for (int i = 0; i < input.scenario.nurseNum; ++i) {
        ...
        ifs >> history.totalAssignNums[nurse] >>
            history.totalWorkingWeekendNums[nurse] >> lastShiftName >>
            history.consecutiveShiftNums[nurse] >>
            history.consecutiveDayNums[nurse] >>
            history.consecutiveDayoffNums[nurse];
        history.lastShifts[nurse] = input.names.shiftMap[lastShiftName];
        if (history.pastWeekCount ==
            0) {  // clear total assign if it is the first week
...
bool readWeekData(const std::string &weekDataFileName, NurseRostering &input) {
    ...
    weekdata.minNurseNums = vector<vector<vector<int> > >(
        NurseRostering::Weekday::SIZE,
        vector<vector<int> >(input.scenario.shiftSize,
                             vector<int>(input.scenario.skillSize)));
    weekdata.optNurseNums = vector<vector<vector<int> > >(
        NurseRostering::Weekday::SIZE,
        vector<vector<int> >(input.scenario.shiftSize,
                             vector<int>(input.scenario.skillSize)));
    weekdata.shiftOffs = vector<vector<vector<bool> > >(
        NurseRostering::Weekday::SIZE,
        vector<vector<bool> >(input.scenario.shiftSize,
                              vector<bool>(input.scenario.nurseNum, false)));
    char c;
    char buf[MAX_BUF_SIZE];
    ...
    while (true) {
        ...
        for (int weekday = NurseRostering::Weekday::Mon;
            ...
            ifs >> c >> weekdata.minNurseNums[weekday][shift][skill] >> c >>
                weekdata.optNurseNums[weekday][shift][skill] >> c;
        }
    }
...
bool readCustomInput(const std::string &customInputFileName,
    ...
    ifs.read(reinterpret_cast<char *>(&history.accObjValue),
             sizeof(history.accObjValue));
    ifs.read(reinterpret_cast<char *>(&history.pastWeekCount),
             sizeof(history.pastWeekCount));
    history.currentWeek = history.pastWeekCount + 1;
    history.restWeekCount = input.scenario.totalWeekNum - history.pastWeekCount;
    // ifs.read( reinterpret_cast<char *>(&history.currentWeek), sizeof(
    // history.currentWeek ) ); ifs.read( reinterpret_cast<char
    // *>(&history.restWeekCount), sizeof( history.restWeekCount ) );
    ifs.read(reinterpret_cast<char *>(totalAssignNums), nurseNum * sizeof(int));
    ifs.read(reinterpret_cast<char *>(totalWorkingWeekendNums),
             nurseNum * sizeof(int));
    ifs.read(reinterpret_cast<char *>(lastShifts),
             nurseNum * sizeof(NurseRostering::ShiftID));
    ifs.read(reinterpret_cast<char *>(consecutiveShiftNums),
             nurseNum * sizeof(int));
    ifs.read(reinterpret_cast<char *>(consecutiveDayNums),
             nurseNum * sizeof(int));
    ifs.read(reinterpret_cast<char *>(consecutiveDayoffNums),
             nurseNum * sizeof(int));

    history.totalAssignNums =
        vector<int>(totalAssignNums, totalAssignNums + nurseNum);
    history.totalWorkingWeekendNums = vector<int>(
        totalWorkingWeekendNums, totalWorkingWeekendNums + nurseNum);
    history.lastShifts = vector<int>(lastShifts, lastShifts + nurseNum);
    history.consecutiveShiftNums =
        vector<int>(consecutiveShiftNums, consecutiveShiftNums + nurseNum);
    history.consecutiveDayNums =
        vector<int>(consecutiveDayNums, consecutiveDayNums + nurseNum);
    history.consecutiveDayoffNums =
        vector<int>(consecutiveDayoffNums, consecutiveDayoffNums + nurseNum);
...
bool writeSolution(const std::string &solutionFileName,
    ...
    const NurseRostering::AssignTable &assign(
        solver.getOptima().getAssignTable());
    ...
    ofs << solver.problem.history.pastWeekCount << ' '
        << solver.problem.names.scenarioName << endl
        << endl;
    int totalAssign = 0;
    ostringstream oss;
    for (NurseRostering::NurseID nurse = 0;
         nurse < solver.problem.scenario.nurseNum; ++nurse) {
        for (int weekday = NurseRostering::Weekday::Mon;
             weekday <= NurseRostering::Weekday::Sun; ++weekday) {
            if (assign.isWorking(nurse, weekday)) {
                ++totalAssign;
                oss << names.nurseNames[nurse] << ' ' << weekdayNames[weekday]
...
bool writeCustomOutput(const std::string &customOutputFileName,
    ...
    ofs.write(reinterpret_cast<const char *>(&history.accObjValue),
              sizeof(history.accObjValue));
    ofs.write(reinterpret_cast<const char *>(&history.pastWeekCount),
              sizeof(history.pastWeekCount));
    // ofs.write( reinterpret_cast<const char *>(&history.currentWeek), sizeof(
    // history.currentWeek ) ); ofs.write( reinterpret_cast<const char
    // *>(&history.restWeekCount), sizeof( history.restWeekCount ) );
    ofs.write(reinterpret_cast<const char *>(history.totalAssignNums.data()),
              history.totalAssignNums.size() * sizeof(int));
    ofs.write(
        reinterpret_cast<const char *>(history.totalWorkingWeekendNums.data()),
        history.totalWorkingWeekendNums.size() * sizeof(int));
    ofs.write(reinterpret_cast<const char *>(history.lastShifts.data()),
              history.lastShifts.size() * sizeof(NurseRostering::ShiftID));
    ofs.write(
        reinterpret_cast<const char *>(history.consecutiveShiftNums.data()),
        history.consecutiveShiftNums.size() * sizeof(int));
    ofs.write(reinterpret_cast<const char *>(history.consecutiveDayNums.data()),
              history.consecutiveDayNums.size() * sizeof(int));
    ofs.write(
        reinterpret_cast<const char *>(history.consecutiveDayoffNums.data()),
        history.consecutiveDayoffNums.size() * sizeof(int));
...
NurseRostering::Solver::Config parseConfig(const std::string &configString) {
    ...
    iss >> c;
    if (c == 'w') {
        config.solveAlgorithm =
            NurseRostering::Solver::SolveAlgorithm::RandomWalk;
    } else if (c == 'i') {
        config.solveAlgorithm =
            NurseRostering::Solver::SolveAlgorithm::IterativeLocalSearch;
    } else if (c == 'p') {
        config.solveAlgorithm =
            NurseRostering::Solver::SolveAlgorithm::TabuSearch_Possibility;
    } else if (c == 'l') {
        config.solveAlgorithm =
            NurseRostering::Solver::SolveAlgorithm::TabuSearch_Loop;
    } else if (c == 'r') {
        config.solveAlgorithm =
            NurseRostering::Solver::SolveAlgorithm::TabuSearch_Rand;
    } else if (c == 'b') {
        config.solveAlgorithm =
            NurseRostering::Solver::SolveAlgorithm::BiasTabuSearch;
    } else if (c == 's') {
        config.solveAlgorithm =
            NurseRostering::Solver::SolveAlgorithm::SwapChainSearch;
    } else {
        return config;
    }
    ...
    if ((modeSeq >= 0) && (modeSeq < NurseRostering::Solution::ModeSeq::SIZE)) {
        config.modeSeq =
            static_cast<NurseRostering::Solution::ModeSeq>(modeSeq);
    }
    ...
    iss >> c >>
        config.dayTabuCoefficient
            [NurseRostering::Solver::TabuTenureCoefficientIndex::TableSize] >>
        c >>
        config.dayTabuCoefficient
            [NurseRostering::Solver::TabuTenureCoefficientIndex::NurseNum] >>
        c >> config.dayTabuCoefficient
                 [NurseRostering::Solver::TabuTenureCoefficientIndex::DayNum] >>
        c >>
        config.dayTabuCoefficient
            [NurseRostering::Solver::TabuTenureCoefficientIndex::ShiftNum] >>
        c >>
        config.shiftTabuCoefficient
            [NurseRostering::Solver::TabuTenureCoefficientIndex::TableSize] >>
        c >>
        config.shiftTabuCoefficient
            [NurseRostering::Solver::TabuTenureCoefficientIndex::NurseNum] >>
        c >> config.shiftTabuCoefficient
                 [NurseRostering::Solver::TabuTenureCoefficientIndex::DayNum] >>
        c >> config.shiftTabuCoefficient
                 [NurseRostering::Solver::TabuTenureCoefficientIndex::ShiftNum];
