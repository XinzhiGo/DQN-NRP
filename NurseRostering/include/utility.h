...
class Timer {
   public:
    ...
    bool isTimeOut() const { return (Clock::now() >= endTime); }

    Duration restTime() const {
        return std::chrono::duration_cast<Duration>(endTime - Clock::now());
    }
...
class RandSelect {
   public:
    ...
    // so you can pass 1 into the constructor in this condition to leave out a
    // isSelected() call.
    RandSelect(int startCount = 1) : count(startCount) {}

    // start a new selection on another N elements
    void reset(int startCount = 1) { count = startCount; }

    // call this for each of the N elements (N times in total) to judge whether
    // each of them is selected.
    bool isSelected(
        std::mt19937 &randGen) {  // only the last returned "true" means that
                                  // element is selected finally.
