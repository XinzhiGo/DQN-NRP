 ...
 *
 *   note :  v1.  use repair mode of penalty to fix infeasible solution in
 * repair(). v2.  merge add and remove to a single neighborhood, abreast of swap
 * and change. x3.  const solver will not be able to call updateOptima, but
 * none-const solver will stub the construction in solver.genHistory().
 *           4.  create a fastRepair() which is separate repair and local search
 *               to improve efficiency since repair process is more similar to
 * generating initial assignment? v5.  perturb() should start from optima? v6.
 * move mode setting to findBest rather than try.
 *           7.  just add max tabu tenure to iterCount rather than reset tabu
 * table when rebuild (except the first time)
 *           8.  rebuild() may not function well if diff is too big or small.
 *           v9.  there will be problem on total assignment violation
 * calculation if total assignment in initial history is not 0.
 *
 */
...
class NurseRostering::Output {
   public:
    ...
    Output()
        : objValue(DefaultPenalty::FORBIDDEN_MOVE),
          secondaryObjValue(DefaultPenalty::FORBIDDEN_MOVE) {}
    Output(ObjValue objVal, const AssignTable &assignment,
           double secondaryObjVal = DefaultPenalty::FORBIDDEN_MOVE,
           Timer::TimePoint findAssignTime = std::chrono::steady_clock::now())
        : objValue(objVal),
          secondaryObjValue(secondaryObjVal),
          assign(assignment),
          findTime(findAssignTime) {}
...
class NurseRostering::Solution : public NurseRostering::Output {
   public:
    ...
    class Move {
       public:
        // fundamental move modes in local search, SIZE is the number of move
        // types AR stands for "Add and Remove", "Rand" means select one to
        // search randomly, "Both" means search both, "Loop" means switch to
        // another when no improvement
        enum Mode {
            ...
            Exchange = ATOMIC_MOVE_SIZE,
            Swap,
            BlockSwap,
            BASIC_MOVE_SIZE,
            // compound moves which can not be used by a single tryXXX() and no
            // apply() for them. apply them by calling apply() of corresponding
            // basic move
            BlockShift = BASIC_MOVE_SIZE,
            ARLoop,
            ARRand,
            ARBoth,
            SIZE
    ...
    typedef void (Solution::*TabuSearch)(
    ...
    class NRPEnv {
       public:
        NRPEnv(NurseRostering::Solution &sln,const FindBestMoveTable &findBestMoveTable,int maxNoImprove);
        torch::Tensor toTensor(NurseRostering::Solution &solution);
        torch::Tensor reset();
        std::tuple<torch::Tensor, double, bool> step(int action);
        NurseRostering::Solution &getSolution() { return sln_; }
        NurseRostering::Output &getOptima() { return optima_; }

       private:
        const FindBestMoveTable &findBestMoveTable_;
        NurseRostering::Output initSln_;
        NurseRostering::Output optima_;
        NurseRostering::Solution &sln_;
    ...
    class Agent {
       public:
        ...
        Agent(int num_inputs, int num_actions, int hidden_size,
              ...
              double tau,NurseRostering::Solution &sln,const FindBestMoveTable &findBestMoveTable,
              int maxNoImprove,std::string qNetFilePath,std::string qTargetFilePath);
        ...
        NurseRostering::Output getGlobalOptima();
    ...
    // set assign to output.assignTable and rebuild assist data, at must be
    // build from same problem
    void rebuild(const Output &output,
                 double diff);           // objValue will be recalculated
    ...
    bool genInitAssign_Dynamic();
    int selectNurse(int weekday,std::vector<std::vector<int>> nurseVector,std::vector<int> finalSelect,int shift,int skill,std::vector<bool> flag);
    ...
    void adjustWeightToBiasNurseWithGreaterPenalty(int inverseTotalBiasRatio,
                                                   int inversePenaltyBiasRatio);
    void dqnSearch(const Timer &timer,const FindBestMoveTable &findBestMoveTable, IterCount maxNoImproveCount);
    void dqnSearchDecetralization(const Timer &timer,const FindBestMoveTable &findBestMoveTable,NurseRostering::IterCount maxNoImproveCount);
    void compareSearch(const Timer &timer,const FindBestMoveTable &findBestMoveTable,NurseRostering::IterCount maxNoImproveCount);
    void VNDSearch(const Timer &timer,const FindBestMoveTable &findBestMoveTable,NurseRostering::IterCount maxNoImproveCount);
    ...
    void swapChainSearch_DoubleHead(const Timer &timer,
                                    IterCount maxNoImproveChainLength);
    ...
    // or block shift which will stop the chain if improved. the second one is
    // to find a block swap to improve this nurse which will continue the chain.
    bool genSwapChain(const Timer &timer, const Move &head,
    ...
    void tabuSearch_Rand(const Timer &timer,
                         const FindBestMoveTable &findBestMoveTable,
                         IterCount maxNoImproveCount);
    void VNDSearch_Rand(const Timer &timer,
                         const FindBestMoveTable &findBestMoveTable,
                         IterCount maxNoImproveCount);
    void tabuSearch_Rand(const FindBestMoveTable &findBestMoveTable,
                         IterCount maxIterCount);
    void VNDSearch_Rand(const FindBestMoveTable &findBestMoveTable,
                         IterCount maxIterCount);
    double  applyBestMoveTable(int action,const FindBestMoveTable &findBestMoveTable);
    ...
    void tabuSearch_Loop(const Timer &timer,
                         const FindBestMoveTable &findBestMoveTable,
                         IterCount maxNoImproveCount);
    ...
    void tabuSearch_Possibility(const Timer &timer,
                                const FindBestMoveTable &findBestMoveTable,
                                IterCount maxNoImproveCount);
    ...
    void localSearch(const Timer &timer,
                     const FindBestMoveTable &findBestMoveTable);
    ...
    // check if the result of incremental update, evaluate and checkObjValue is
    // the same
   ...
   private:
    ...
    class Consecutive {
       public:
        static const int SIZE =
            Weekday::Sun + 1;  // no need for a slot for next week
        ...
        Consecutive(const History &his, NurseID nurse) {
            if (Assign::isWorking(his.lastShifts[nurse])) {
                ...
                std::fill(dayLow, dayLow + SIZE,
                          1 - his.consecutiveDayoffNums[nurse]);
                std::fill(dayHigh, dayHigh + SIZE, Weekday::Sun);
                std::fill(shiftLow, shiftLow + SIZE,
                          1 - his.consecutiveDayoffNums[nurse]);
                std::fill(shiftHigh, shiftHigh + SIZE, Weekday::Sun);
            }
        }
        ...
        bool isSingleConsecutiveShift()
            const {  // shiftLow may be HIS, so use less or equal
            return (shiftLow[Weekday::Sun] <= Weekday::Mon);
        }
        // if a day or day-off lasts whole week, return true, else false
        bool isSingleConsecutiveDay()
            const {  // dayLow may be HIS, so use less or equal
    ...
    // (iterCount <= ShiftTabu[nurse][weekday][shift][skill]) means forbid to be
    // added
    typedef std::vector<std::vector<std::vector<std::vector<IterCount> > > >
        ShiftTabu;
    ...
    static ObjValue penaltyDayNum(int len, int end, int min, int max) {
        return (end < Weekday::Sun) ? distanceToRange(len, min, max)
                                    : exceedCount(len, max);
    }

    // depth first search to fill assign
    bool fillAssign(int weekday, ShiftID shift, SkillID skill, NurseID nurse,
                    int nurseNum);
    ...
    bool isValidSuccession(NurseID nurse, ShiftID shift, int weekday) const {
        return problem.scenario.shifts[assign[nurse][weekday - 1].shift]
            .legalNextShifts[shift];
    }
    bool isValidPrior(NurseID nurse, ShiftID shift, int weekday) const {
        return problem.scenario.shifts[shift]
            .legalNextShifts[assign[nurse][weekday + 1].shift];
    }
    ...
    void applyBasicMove(const Move &move) {
        ...
        std::cerr << objValue << ","
                  << AssignTable::distance(this->getAssignTable(),
    ...
    bool updateOptima() {
        ...
        if (objValue <= optima.getObjValue()) {
            ...
            for (NurseID n = 0; n < problem.scenario.nurseNum; ++n) {
                const NurseRostering::Scenario::Nurse &nurse(
                    problem.scenario.nurses[n]);
                secondaryObjValue +=
                    (static_cast<double>(totalAssignNums[n]) /
                     (1 + abs(nurse.restMaxShiftNum +
                              problem.scenario.contracts[nurse.contract]
                                  .maxShiftNum)));
            }
        }
#endif
...
#ifdef INRC2_SECONDARY_OBJ_VALUE
            bool isSelected =
                (secondaryObjValue < optima.getSecondaryObjValue());
#else
    ...
    bool findBestBlockSwap_part(
        Move &bestMove) const;  // try some nurses following index
    bool findBestBlockSwap_rand(
        Move &bestMove) const;  // try randomly picked nurses
    ...
    // evaluate cost of assigning another Assign or skill to nurse already
    // assigned in weekday
    ObjValue tryChangeAssign(int weekday, NurseID nurse, const Assign &a) const;
    ObjValue tryChangeAssign(const Move &move) const;
    // evaluate cost of removing the Assign from nurse already assigned in
    // weekday
    ...
    // evaluate cost of swapping Assign of two nurses in consecutive days start
    // from weekday and record the selected end of the block into weekday2 the
    // recorded move will always be no tabu move or meet aspiration criteria
    ObjValue trySwapBlock(int weekday, int &weekday2, NurseID nurse,
                          NurseID nurse2) const;
    ObjValue trySwapBlock(const Move &move) const;
    // evaluate cost of swapping Assign of two nurses in consecutive days in a
    // week and record the block information into weekday and weekday2 the
    // recorded move will always be no tabu move or meet aspiration criteria
    ObjValue trySwapBlock_fast(int &weekday, int &weekday2, NurseID nurse,
                               NurseID nurse2) const;
    ...
    // apply assigning another Assign or skill to nurse already assigned in
    // weekday
    ...
    // apply swapping Assign of two nurses in consecutive days within [weekday,
    // weekday2]
    ...
    void assignSingle(int weekday, int high[], int low[], bool affectRight,
                      bool affectLeft);

#ifdef INRC2_USE_TABU
    bool noAddTabu(const Move &move) const {
        return (iterCount > shiftTabu[move.nurse][move.weekday]
                                     [move.assign.shift][move.assign.skill]);
    }
    ...
    bool noSwapTabu(int weekday, NurseID nurse, NurseID nurse2) const {
        ...
        if (a.isWorking()) {
            if (a2.isWorking()) {
                return (
                    (iterCount >
                     shiftTabu[nurse][weekday][a2.shift][a2.skill]) ||
                    (iterCount > shiftTabu[nurse2][weekday][a.shift][a.skill]));
            } else {
                return (
                    (iterCount > dayTabu[nurse][weekday]) ||
                    (iterCount > shiftTabu[nurse2][weekday][a.shift][a.skill]));
            }
        } else {
            if (a2.isWorking()) {
    ...
    bool noExchangeTabu(const Move &move) const {
        ...
        if (a.isWorking()) {
            if (a2.isWorking()) {
                return (
                    (iterCount >
                     shiftTabu[move.nurse][move.weekday][a2.shift][a2.skill]) ||
                    (iterCount >
                     shiftTabu[move.nurse][move.weekday2][a.shift][a.skill]));
            } else {
                return (
                    (iterCount > dayTabu[move.nurse][move.weekday]) ||
                    (iterCount >
                     shiftTabu[move.nurse][move.weekday2][a.shift][a.skill]));
            }
        } else {
    ...
    bool aspirationCritiera(ObjValue bestMoveDelta,
                            ObjValue bestMoveDelta_tabu) const {
    ...
    void updateChangeTabu(const Move &move) {
        updateShiftTabu(move.nurse, move.weekday,
                        assign[move.nurse][move.weekday]);
    }
    ...
    mutable bool
        findBestARLoopOnBlockBorder_flag;  // findBestARLoopOnBlockBorder()
                                           // will modify it
    // for controlling start point of the search of best block swap
    mutable NurseID
        findBestBlockSwap_startNurse;  // findBestBlockSwap() will modify it
    // TODO : remove following two variables if TSP is not used
    // for controlling swap and block swap will not be selected both in
    // possibility select
    bool isPossibilitySelect;
    mutable bool isBlockSwapSelected;  // tabuSearch_Possibility() and
                                       // findBestBlockSwap() wil modify it
    // trySwapNurse() and trySwapBlock() will guarantee they are always
    // corresponding to the selected swap
    mutable ObjValue
        nurseDelta;  // trySwapNurse() and trySwapBlock() will modify it
    mutable ObjValue
        nurse2Delta;  // trySwapNurse() and trySwapBlock() will modify it
