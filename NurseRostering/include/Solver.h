 ...
 *           v4. record 8 day which the first day is last day of last week to
 * unify the succession judgment. x5. [optimizable] put evaluateObjValue() in
 * initAssign() or call it in init() ?
 *           6.  [optimizable] evaluateConsecutiveDay() and
 * evaluateConsecutiveDayOff() can be put together and not consider if there is
 * an assignment. but there is a difficult problem with consecutive state on the
 * beginning of the week. x7.  [optimizable] make Weekday not related to
 * Consecutive count from 0 to save space. v8.  [optimizable] make shiftID count
 * from 1 and make Shift::ID_NONE 0, this will leave out isWorking() in
 * isValidSuccesion(). also, add a slot after Sun in assign with shift ID_NONE
 * to leave out (weekday >= Weekday::Sun) in isValidPrior()
 *           9.  timeout may overflow in POSIX system since CLOCKS_PER_SEC is
 * 1000000 which makes clock_t can only count to about 4000 seconds. v10.
 * record() uses c style function to check file open state, change it to c++
 * style. v11. reset solution to global optima or last local optima after each
 * tabuSearch?
 *           12. use constructor with assign string of AssignTable to rebuild
 * solution.
 *
 */
...
class NurseRostering::Solver {
   public:
    ...
    // inverse possibility of starting bias tabu search from optima in last
    // search
    ...
    enum InitAlgorithm { Greedy, Exact };

    enum SolveAlgorithm {
        RandomWalk,
        IterativeLocalSearch,
        TabuSearch_Possibility,
        TabuSearch_Loop,
        TabuSearch_Rand,
        BiasTabuSearch,
        DQNSearch,
    ...
    class Config {
       public:
        Config()
            : initAlgorithm(InitAlgorithm::Greedy),
    ...
    Solver(const NurseRostering &input, const Output &optima,
           Timer::TimePoint startTime);
    virtual ~Solver() {}

    // set algorithm name, set parameters, generate initial solution
    virtual void init(const Config &cfg = Config(),
                      const std::string &runID = std::string()) = 0;
    // search for optima
    ...
    void record(const std::string logFileName,
                const std::string &instanceName) const;  // contain check()
    ...
    const NurseNumOfSkill &getNurseNumOfSkill() const {
        return nurseNumOfSkill;
    }
   ...
   protected:
    ...
    void checkConsecutiveViolation(int &objValue, const AssignTable &assign,
                                   NurseID nurse, int weekday,
                                   ShiftID lastShiftID, int &consecutiveShift,
                                   int &consecutiveDay, int &consecutiveDayOff,
                                   bool &shiftBegin, bool &dayBegin,
...
class NurseRostering::TabuSolver : public NurseRostering::Solver {
   public:
    ...
    TabuSolver(const NurseRostering &input, const Output &optima,
               Timer::TimePoint startTime = Timer::Clock::now());
    virtual ~TabuSolver() {}
    ...
    void checkDump(std::string assignString) {
        sln.rebuild(Output(0, AssignTable(problem.scenario.nurseNum,
                                          Weekday::SIZE, assignString)));
    ...
    IterCount MaxNoImproveForSingleNeighborhood() const {
        return maxNoImproveForSingleNeighborhood;
    }
    IterCount MaxNoImproveForAllNeighborhood() const {
        return maxNoImproveForAllNeighborhood;
    }
    IterCount MaxNoImproveForBiasTabuSearch() const {
        return maxNoImproveForBiasTabuSearch;
    }
    IterCount MaxNoImproveSwapChainLength() const {
        return maxNoImproveSwapChainLength;
    }
    IterCount MaxSwapChainRestartCount() const {
        return maxSwapChainRestartCount;
    }
   ...
   private:
    ...
    void DQNSearch(Solution::ModeSeq modeSeq);
    void CompareSearch(Solution::ModeSeq modeSeq);
    // search with tabu table
    void tabuSearch(Solution::ModeSeq modeSeq, Solution::TabuSearch search,
    ...
    void setMaxNoImprove(double coefficient) {
        ...
        maxNoImproveForAllNeighborhood = static_cast<IterCount>(
            coefficient * problem.scenario.nurseNum * Weekday::NUM *
            sqrt(problem.scenario.shiftTypeNum *
                 problem.scenario.skillTypeNum));
        maxNoImproveForBiasTabuSearch =
            maxNoImproveForSingleNeighborhood / INVERSE_TOTAL_BIAS_RATIO;
        maxNoImproveSwapChainLength = maxNoImproveForSingleNeighborhood;
        maxSwapChainRestartCount =
            static_cast<IterCount>(sqrt(problem.scenario.nurseNum));
    }
