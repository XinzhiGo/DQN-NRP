...
const Timer::Duration NurseRostering::Solver::SAVE_SOLUTION_TIME =
    Timer::Duration(250);
...
NurseRostering::Solver::Solver(const NurseRostering &input, const Output &opt,
                               Timer::TimePoint st)
    : problem(input),
      startTime(st),
      optima(opt),
      timer(problem.timeout, startTime) {}
...
NurseRostering::ObjValue NurseRostering::Solver::checkFeasibility(
    const AssignTable &assign) const {
    ObjValue objValue = 0;
    ...
    for (int weekday = Weekday::Mon; weekday <= Weekday::Sun; ++weekday) {
        for (ShiftID shift = NurseRostering::Scenario::Shift::ID_BEGIN;
            ...
            for (SkillID skill = NurseRostering::Scenario::Skill::ID_BEGIN;
                ...
                if (nurseNum[weekday][shift][skill] <
    ...
    for (int weekday = Weekday::Mon; weekday <= Weekday::Sun; ++weekday) {
        for (NurseID nurse = 0; nurse < problem.scenario.nurseNum; ++nurse) {
            objValue +=
                ...
                (!problem.scenario.shifts[assign[nurse][weekday - 1].shift]
                      .legalNextShifts[assign[nurse][weekday].shift]);
        }
    }
    ...
    for (NurseID nurse = 0; nurse < problem.scenario.nurseNum; ++nurse) {
        for (int weekday = Weekday::Mon; weekday <= Weekday::Sun; ++weekday) {
            if (assign[nurse][weekday].isWorking() &&
                !problem.scenario.nurses[nurse]
                     .skills[assign[nurse][weekday].skill]) {
...
NurseRostering::ObjValue NurseRostering::Solver::checkObjValue(
    const AssignTable &assign) const {
    ObjValue objValue = 0;
    ...
    for (int weekday = Weekday::Mon; weekday <= Weekday::Sun; ++weekday) {
        for (ShiftID shift = NurseRostering::Scenario::Shift::ID_BEGIN;
            ...
            for (SkillID skill = NurseRostering::Scenario::Skill::ID_BEGIN;
                 skill < problem.scenario.skillSize; ++skill) {
                int missingNurse =
                    (problem.weekData.optNurseNums[weekday][shift][skill] -
                     nurseNums[weekday][shift][skill]);
                if (missingNurse > 0) {
                    objValue +=
                        DefaultPenalty::InsufficientStaff * missingNurse;
                }
            }
        }
    }
    ...
    for (NurseID nurse = 0; nurse < problem.scenario.nurseNum; ++nurse) {
        ...
        checkConsecutiveViolation(
            objValue, assign, nurse, Weekday::Mon,
            problem.history.lastShifts[nurse], consecutiveShift, consecutiveDay,
            consecutiveDayOff, shiftBegin, dayBegin, dayoffBegin);

        for (int weekday = Weekday::Tue; weekday <= Weekday::Sun; ++weekday) {
            checkConsecutiveViolation(objValue, assign, nurse, weekday,
                                      assign[nurse][weekday - 1].shift,
                                      consecutiveShift, consecutiveDay,
                                      consecutiveDayOff, shiftBegin, dayBegin,
                                      dayoffBegin);
        }
        // since penalty was calculated when switching assign, the penalty of
        // last consecutive assignments are not considered. so finish it here.
        const ContractID &contractID(problem.scenario.nurses[nurse].contract);
        const Scenario::Contract &contract(
            problem.scenario.contracts[contractID]);
        if (dayoffBegin && problem.history.consecutiveDayoffNums[nurse] >
            ...
            if (shiftBegin &&
                problem.history.consecutiveShiftNums[nurse] >
                    problem.scenario.shifts[assign[nurse][Weekday::Sun].shift]
                        .maxConsecutiveShiftNum) {
                objValue += DefaultPenalty::ConsecutiveShift * Weekday::NUM;
            } else if (consecutiveShift >
                       problem.scenario
                           .shifts[assign[nurse][Weekday::Sun].shift]
                           .maxConsecutiveShiftNum) {
                objValue +=
            ...
            if (dayBegin && problem.history.consecutiveDayNums[nurse] >
    ...
    if (problem.history.currentWeek == problem.scenario.totalWeekNum) {
        for (NurseID nurse = 0; nurse < problem.scenario.nurseNum; ++nurse) {
            ...
            int min = problem.scenario
                          .contracts[problem.scenario.nurses[nurse].contract]
                          .minShiftNum;
            int max = problem.scenario
                          .contracts[problem.scenario.nurses[nurse].contract]
                          .maxShiftNum;
            objValue +=
                DefaultPenalty::TotalAssign *
                distanceToRange(assignNum * problem.scenario.totalWeekNum,
                                min * problem.history.currentWeek,
                                max * problem.history.currentWeek) /
            ...
            objValue +=
                DefaultPenalty::TotalAssign *
                distanceToRange(assignNum * problem.history.restWeekCount, min,
                                max) /
            ...
            int historyWeekend =
                problem.history.totalWorkingWeekendNums[nurse] *
                problem.scenario.totalWeekNum;
            ...
            if (exceedingWeekend > 0) {
                objValue += DefaultPenalty::TotalWorkingWeekend *
            ...
            int workingWeekendNum = (assign.isWorking(nurse, Weekday::Sat) ||
                                     assign.isWorking(nurse, Weekday::Sun));
            objValue +=
                DefaultPenalty::TotalWorkingWeekend *
                exceedCount(
...
void NurseRostering::Solver::print() const {
    cout << "optima.objVal: " << (optima.getObjValue() / DefaultPenalty::AMP)
         << endl;
}

void NurseRostering::Solver::initResultSheet(std::ofstream &csvFile) {
    csvFile << "Time,ID,Instance,Algorithm,RandSeed,GenCount,IterCount,"
               "Duration,Feasible,Check-Obj,ObjValue,AccObjValue,Solution"
            << std::endl;
}

void NurseRostering::Solver::record(const std::string logFileName,
    ...
    csvFile << getTime() << "," << runID << "," << instanceName << ","
            << algorithmName << "," << problem.randSeed << ","
            << generationCount << "," << iterationCount << ","
            << chrono::duration_cast<std::chrono::milliseconds>(
                   optima.getFindTime() - startTime)
                       .count() /
                   1000.0
            << "s," << checkFeasibility() << ","
            << (checkObjValue() - optima.getObjValue()) /
                   static_cast<double>(DefaultPenalty::AMP)
            << ","
            << optima.getObjValue() / static_cast<double>(DefaultPenalty::AMP)
            << ","
            << (optima.getObjValue() + problem.history.accObjValue) /
                   static_cast<double>(DefaultPenalty::AMP)
            << ",";
...
NurseRostering::NurseNumsOnSingleAssign NurseRostering::Solver::countNurseNums(
    const AssignTable &assign) const {
    NurseNumsOnSingleAssign nurseNums(
        Weekday::SIZE,
        vector<vector<int> >(problem.scenario.shiftSize,
                             vector<int>(problem.scenario.skillSize, 0)));
    for (NurseID nurse = 0; nurse < problem.scenario.nurseNum; ++nurse) {
        for (int weekday = Weekday::Mon; weekday <= Weekday::Sun; ++weekday) {
            ++nurseNums[weekday][assign[nurse][weekday].shift]
...
void NurseRostering::Solver::checkConsecutiveViolation(
    int &objValue, const AssignTable &assign, NurseID nurse, int weekday,
    ShiftID lastShiftID, int &consecutiveShift, int &consecutiveDay,
    int &consecutiveDayOff, bool &shiftBegin, bool &dayBegin,
    bool &dayoffBegin) const {
    const ContractID &contractID = problem.scenario.nurses[nurse].contract;
    const Scenario::Contract &contract(problem.scenario.contracts[contractID]);
    const ShiftID &shift = assign[nurse][weekday].shift;
    if (Assign::isWorking(shift)) {  // working day
        if (consecutiveDay ==
            0) {  // switch from consecutive day off to working
            if (dayoffBegin) {
                if (problem.history.consecutiveDayoffNums[nurse] >
                    contract.maxConsecutiveDayoffNum) {
                    objValue += DefaultPenalty::ConsecutiveDayOff *
                                (weekday - Weekday::Mon);
                } else {
                    objValue +=
                        ...
                        distanceToRange(consecutiveDayOff,
            ...
            if (shift == lastShiftID) {
                ...
                const Scenario::Shift &lastShift(
                    problem.scenario.shifts[lastShiftID]);
                if (shiftBegin) {
                    if (problem.history.consecutiveShiftNums[nurse] >
                        lastShift.maxConsecutiveShiftNum) {
                        objValue += DefaultPenalty::ConsecutiveShift *
                                    (weekday - Weekday::Mon);
                    } else {
                        objValue +=
                            DefaultPenalty::ConsecutiveShift *
                            distanceToRange(consecutiveShift,
                                            lastShift.minConsecutiveShiftNum,
                                            lastShift.maxConsecutiveShiftNum);
                    }
                    shiftBegin = false;
                } else {
        ...
        if (consecutiveDayOff ==
            0) {  // switch from consecutive working to day off
            const Scenario::Shift &lastShift(
                problem.scenario.shifts[lastShiftID]);
            if (shiftBegin) {
                if (problem.history.consecutiveShiftNums[nurse] >
            ...
            if (dayBegin) {
                if (problem.history.consecutiveDayNums[nurse] >
                    contract.maxConsecutiveDayNum) {
                    objValue += DefaultPenalty::ConsecutiveDay *
                                (weekday - Weekday::Mon);
                } else {
                    objValue += DefaultPenalty::ConsecutiveDay *
                                distanceToRange(consecutiveDay,
...
void NurseRostering::Solver::discoverNurseSkillRelation() {
    ...
    nurseWithSkill =
        vector<vector<vector<NurseID> > >(problem.scenario.skillSize);
    nursesHasSameSkill = vector<vector<bool> >(
        problem.scenario.nurseNum, vector<bool>(problem.scenario.nurseNum));
    ...
    for (NurseID nurse = 0; nurse < problem.scenario.nurseNum; ++nurse) {
        ...
        for (NurseID nurse2 = 0; nurse2 < problem.scenario.nurseNum; ++nurse2) {
            nursesHasSameSkill[nurse][nurse2] =
                problem.haveSameSkill(nurse, nurse2);
        }
    }
}

NurseRostering::TabuSolver::TabuSolver(const NurseRostering &input,
                                       Timer::TimePoint st)
    : Solver(input, st),
      sln(*this),
      dayTabuTenureBase(1),
      shiftTabuTenureBase(1) {}

NurseRostering::TabuSolver::TabuSolver(const NurseRostering &input,
                                       const Output &opt, Timer::TimePoint st)
    : Solver(input, opt, st),
      sln(*this),
      dayTabuTenureBase(1),
      shiftTabuTenureBase(1) {}

void NurseRostering::TabuSolver::init(const Config &cfg,
                                      const std::string &id) {
    config = cfg;
    runID = id;
    algorithmName = "Tabu";
    ...
    switch (config.solveAlgorithm) {
        ...
        case SolveAlgorithm::TabuSearch_Loop:
            tabuSearch(config.modeSeq, &Solution::tabuSearch_Loop,
                       MaxNoImproveForSingleNeighborhood());
            break;
        case SolveAlgorithm::TabuSearch_Possibility:
            tabuSearch(config.modeSeq, &Solution::tabuSearch_Possibility,
                       MaxNoImproveForAllNeighborhood());
            break;
        case SolveAlgorithm::BiasTabuSearch:
            biasTabuSearch(config.modeSeq);
            break;
        case SolveAlgorithm::DQNSearch:
            DQNSearch(config.modeSeq);
            break;
        case SolveAlgorithm::CompareSearch:
            CompareSearch(config.modeSeq);
            break;
...
bool NurseRostering::TabuSolver::updateOptima(const Output &localOptima) {
    if (localOptima.getObjValue() < optima.getObjValue()) {
        ...
        bool isSelected = (localOptima.getSecondaryObjValue() <
                           optima.getSecondaryObjValue());
#else
...
void NurseRostering::TabuSolver::greedyInit() {
    ...
    if (sln.genInitAssign(static_cast<int>(sqrt(problem.scenario.nurseNum)) +
                          1) == false) {
        errorLog("fail to generate feasible init solution.");
    }
}

void NurseRostering::TabuSolver::dynamicInit() {
    algorithmName += "[DynamicInit]";
    sln.genInitAssign_Dynamic();
...
void NurseRostering::TabuSolver::tabuSearch(Solution::ModeSeq modeSeq,
    ...
    while (!timer.isTimeOut() && (iterationCount < problem.maxIterCount)) {
        ...
        const Output &output(
            (randGen() % PERTURB_ORIGIN_SELECT) ? optima : sln.getOptima());
...
void NurseRostering::TabuSolver::biasTabuSearch(Solution::ModeSeq modeSeq) {
    ...
    int cnt = 0;
    while (cnt < 20) {
        //cout <<"1:  " << sln.getObjValue()/DefaultPenalty::AMP << endl;
        //if(problem.history.currentWeek != problem.scenario.totalWeekNum) {
        sln.dqnSearch(timer, fbmt, MaxNoImproveForAllNeighborhood());
        //}else{
        //sln.tabuSearch_Rand(timer, fbmt, MaxNoImproveForAllNeighborhood());
        //}
        //cout << cnt << endl;
        cnt++;
        //sln.compareSearch(timer, fbmt, MaxNoImproveForAllNeighborhood());
        //sln.VNDSearch(timer, fbmt, MaxNoImproveForAllNeighborhood());
        //sln.tabuSearch_Rand(timer, fbmt, MaxNoImproveForAllNeighborhood());
        //sln.VNDSearch_Rand(timer, fbmt, MaxNoImproveForAllNeighborhood());
        //cout <<"2:  " << sln.getObjValue()/DefaultPenalty::AMP << endl;
        sln.rebuild(optima);

        updateOptima(sln.getOptima());
        const Output &output(
            (randGen() % BIAS_TS_ORIGIN_SELECT) ? optima : sln.getOptima());
        sln.rebuild(output);

        sln.adjustWeightToBiasNurseWithGreaterPenalty(
            INVERSE_TOTAL_BIAS_RATIO, INVERSE_PENALTY_BIAS_RATIO);
        //cout <<"3:  " << sln.getObjValue()/DefaultPenalty::AMP << endl;
        sln.tabuSearch_Rand(timer, fbmt, MaxNoImproveForBiasTabuSearch());
        //sln.VNDSearch_Rand(timer, fbmt, MaxNoImproveForBiasTabuSearch());
        //sln.dqnSearchDecetralization(timer,fbmt,MaxNoImproveForBiasTabuSearch());
        //cout <<"4:  " << sln.getObjValue()/DefaultPenalty::AMP << endl;
        ++generationCount;
        sln.rebuild(sln.getOptima());
        //cout <<"5:  " << sln.getObjValue()/DefaultPenalty::AMP << endl;
...
void NurseRostering::TabuSolver::DQNSearch(Solution::ModeSeq modeSeq) {
    const vector<int> &modeSeqPat(Solution::modeSeqPatterns[modeSeq]);
    int modeSeqLen = modeSeqPat.size();
...
void NurseRostering::TabuSolver::CompareSearch(Solution::ModeSeq modeSeq) {
    const vector<int> &modeSeqPat(Solution::modeSeqPatterns[modeSeq]);
    int modeSeqLen = modeSeqPat.size();
    ...
    vector<int> optimalVal;
    for(int i = 0; i < 20; ++i) { //这里的对比算法写的有问题，不应该是一个sln一直在更新，而是应该每次都选择同一个，这样的对比算法才对
        sln.genInitAssign_Greedy();
        sln.tabuSearch_Rand(fbmt, 3000);
        optimalVal.push_back(sln.getOptima().getObjValue());
        if(updateOptima(sln.getOptima())) {
    ...
    for(int i = 0; i < 4; i++){
        for(int j = 0; j < 10; j++){
            cout << optimalVal[i * 10 + j] / DefaultPenalty::AMP << " ";
        }
    }
}
...
void NurseRostering::TabuSolver::setTabuTenure() {
    setDayTabuTenure_TableSize(
        config.dayTabuCoefficient[TabuTenureCoefficientIndex::TableSize]);
    setDayTabuTenure_NurseNum(
        config.dayTabuCoefficient[TabuTenureCoefficientIndex::NurseNum]);
    setDayTabuTenure_DayNum(
        config.dayTabuCoefficient[TabuTenureCoefficientIndex::DayNum]);
    setDayTabuTenure_ShiftNum(
        config.dayTabuCoefficient[TabuTenureCoefficientIndex::ShiftNum]);

    setShiftTabuTenure_TableSize(
        config.shiftTabuCoefficient[TabuTenureCoefficientIndex::TableSize]);
    setShiftTabuTenure_NurseNum(
        config.shiftTabuCoefficient[TabuTenureCoefficientIndex::NurseNum]);
    setShiftTabuTenure_DayNum(
        config.shiftTabuCoefficient[TabuTenureCoefficientIndex::DayNum]);
    setShiftTabuTenure_ShiftNum(
        config.shiftTabuCoefficient[TabuTenureCoefficientIndex::ShiftNum]);

    if (dayTabuTenureBase < MIN_TABU_BASE) {
        dayTabuTenureBase = MIN_TABU_BASE;
    }
    if (shiftTabuTenureBase < MIN_TABU_BASE) {
        shiftTabuTenureBase = MIN_TABU_BASE;
    }
    dayTabuTenureAmp = 1 + dayTabuTenureBase / TABU_BASE_TO_AMP;
    shiftTabuTenureAmp = 1 + shiftTabuTenureBase / TABU_BASE_TO_AMP;
}

void NurseRostering::TabuSolver::setDayTabuTenure_TableSize(
    double coefficient) {
    if (coefficient > 0) {
        ...
        dayTabuTenureBase *= static_cast<IterCount>(
            1 + coefficient * problem.scenario.nurseNum * Weekday::NUM);
    }
}

void NurseRostering::TabuSolver::setShiftTabuTenure_TableSize(
    double coefficient) {
    if (coefficient > 0) {
        std::ostringstream oss;
        ...
        // plus 1 to make sure it will not be 0
        shiftTabuTenureBase *= static_cast<IterCount>(
            1 + coefficient * problem.scenario.nurseNum * Weekday::NUM *
                    problem.scenario.shiftTypeNum *
                    problem.scenario.skillTypeNum);
    }
}
...
void NurseRostering::TabuSolver::setDayTabuTenure_NurseNum(double coefficient) {
    if (coefficient > 0) {
        ...
        // plus 1 to make sure it will not be 0
        dayTabuTenureBase *=
            static_cast<IterCount>(1 + coefficient * problem.scenario.nurseNum);
    }
}

void NurseRostering::TabuSolver::setShiftTabuTenure_NurseNum(
    double coefficient) {
    if (coefficient > 0) {
        std::ostringstream oss;
...
void NurseRostering::TabuSolver::setDayTabuTenure_DayNum(double coefficient) {
    if (coefficient > 0) {
        ...
        // plus 1 to make sure it will not be 0
        dayTabuTenureBase *=
            static_cast<IterCount>(1 + coefficient * Weekday::NUM);
    }
}
...
void NurseRostering::TabuSolver::setDayTabuTenure_ShiftNum(double coefficient) {
    if (coefficient > 0) {
        ...
        // plus 1 to make sure it will not be 0
        dayTabuTenureBase *= static_cast<IterCount>(
            1 + coefficient * problem.scenario.shiftTypeNum *
                    problem.scenario.skillTypeNum);
    }
}

void NurseRostering::TabuSolver::setShiftTabuTenure_ShiftNum(
    double coefficient) {
    if (coefficient > 0) {
        std::ostringstream oss;
